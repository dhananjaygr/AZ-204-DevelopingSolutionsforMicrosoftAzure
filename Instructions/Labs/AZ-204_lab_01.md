...
1. On the **Storage Account** blade, under settings select **Configuration** change **Allow Blob anonymous access** disabled to **Enabled**. Click on **Save**
...
1. On the **Web App** blade, select **Create** fropdown and select **webapp**.
...
1. In the **Configuration** section, perform the following actions, select **Save**, and then select **Continue**.
    ...
    | **Deployment slot setting** check box | Retain the default value, and then select **OK** to close the pop-up dialog and return to the **Configuration** section and click on **save**|
